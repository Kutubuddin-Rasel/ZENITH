...
const ReleaseDetailModal = ({ open, onClose, release, projectId }: ReleaseDetailModalProps) => {
  ...
  return (
    <Modal open={open} onClose={onClose} title="" maxWidthClass="max-w-4xl">
      {/* Header */}
      <div className="p-6 bg-neutral-50 dark:bg-neutral-800/50 rounded-t-lg">
        <h2 className="text-2xl font-bold text-neutral-900 dark:text-neutral-100">{release.name}</h2>
        <p className="text-sm text-neutral-500 dark:text-neutral-400 mt-1">{release.description}</p>
        <div className="mt-4 flex flex-wrap items-center gap-4 text-sm">
          <span className="text-xs px-2 py-0.5 rounded bg-neutral-200 dark:bg-neutral-700 text-neutral-700 dark:text-neutral-200 capitalize font-medium">{release.status}</span>
          {release.releaseDate && (
      ...
      {/* Stats */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-6 border-b border-neutral-200 dark:border-neutral-700">
        <div className="flex items-center gap-4">
          <div className="relative">
            <svg className="w-20 h-20 transform -rotate-90">
              <circle cx="40" cy="40" r="34" stroke="currentColor" strokeWidth="8" className="text-neutral-200 dark:text-neutral-700" fill="transparent" />
              <circle
            ...
            <span className="absolute inset-0 flex items-center justify-center text-xl font-bold text-neutral-800 dark:text-neutral-200">{stats.percent}%</span>
          </div>
          <div>
            <div className="text-2xl font-bold text-neutral-800 dark:text-neutral-200">{stats.done} / {stats.total}</div>
            <div className="text-sm text-neutral-500 dark:text-neutral-400">Issues Completed</div>
          </div>
        </div>
        <div className="col-span-2">
          <p className="text-sm font-medium text-neutral-600 dark:text-neutral-300 mb-2">Issue Status Breakdown</p>
          <div className="flex flex-wrap gap-2">
            {Object.entries(stats.byStatus).map(([status, count]) => (
              <span key={status} className={`px-2.5 py-1 text-xs font-semibold rounded-full ${statusColors[status as keyof typeof statusColors] || 'bg-neutral-200'}`}>
                {status}: {count}
              </span>
            ))}
          </div>
      ...
      <div className="p-6">
        <div className="mb-4 border-b border-neutral-200 dark:border-neutral-700">
          <nav className="-mb-px flex gap-4 overflow-x-auto">
            <button
              className={`py-2 px-1 font-semibold text-sm transition-colors whitespace-nowrap ${activeTab === 'issues' ? 'text-accent-blue border-b-2 border-accent-blue' : 'text-neutral-500 hover:text-neutral-700 dark:hover:text-neutral-300'}`}
              onClick={() => setActiveTab('issues')}
            >Issues</button>
            <button
              className={`py-2 px-1 font-semibold text-sm transition-colors whitespace-nowrap ${activeTab === 'attachments' ? 'text-accent-blue border-b-2 border-accent-blue' : 'text-neutral-500 hover:text-neutral-700 dark:hover:text-neutral-300'}`}
              onClick={() => setActiveTab('attachments')}
            >Attachments</button>
            <button
              className={`py-2 px-1 font-semibold text-sm transition-colors whitespace-nowrap flex items-center gap-1 ${activeTab === 'notes' ? 'text-accent-blue border-b-2 border-accent-blue' : 'text-neutral-500 hover:text-neutral-700 dark:hover:text-neutral-300'}`}
              onClick={() => setActiveTab('notes')}
            ><DocumentTextIcon className="h-4 w-4" /> Notes</button>
            <button
              className={`py-2 px-1 font-semibold text-sm transition-colors whitespace-nowrap flex items-center gap-1 ${activeTab === 'git' ? 'text-accent-blue border-b-2 border-accent-blue' : 'text-neutral-500 hover:text-neutral-700 dark:hover:text-neutral-300'}`}
              onClick={() => setActiveTab('git')}
            ><CodeBracketIcon className="h-4 w-4" /> Git</button>
            <button
              className={`py-2 px-1 font-semibold text-sm transition-colors whitespace-nowrap flex items-center gap-1 ${activeTab === 'actions' ? 'text-accent-blue border-b-2 border-accent-blue' : 'text-neutral-500 hover:text-neutral-700 dark:hover:text-neutral-300'}`}
        ...
        {activeTab === 'issues' && (
          <div className="space-y-6">
            <div>
              <h4 className="font-semibold mb-2 text-neutral-800 dark:text-neutral-200">Add issue to release</h4>
              <Downshift
                onChange={selection => {
                  if (selection) {
                  ...
                  <div className="relative">
                    <div {...getRootProps({}, { suppressRefError: true })}>
                      <input
                        ...
                        className="w-full p-2 border border-neutral-300 dark:border-neutral-600 rounded-md bg-white dark:bg-neutral-800 focus:ring-2 focus:ring-accent-blue focus:outline-none"
                      />
                    </div>
                    <ul
                      {...getMenuProps()}
                      className={`absolute z-10 w-full mt-1 bg-white dark:bg-neutral-800 shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm ${!isOpen && 'hidden'
                        }`}
                    >
                      {loadingAllIssues ? (
                        <li className="px-4 py-2 text-neutral-500">Loading...</li>
                      ) : (
                        issuesNotInRelease
            ...
            <div>
              <h4 className="font-semibold mb-2 text-neutral-800 dark:text-neutral-200">Issues in this release ({stats.total})</h4>
              {loadingRelease ? (
                <div className="flex justify-center py-8"><Spinner className="h-6 w-6" /></div>
              ) : errorRelease ? (
                <div className="text-red-500 text-center py-8">Failed to load release issues.</div>
              ) : (
                <ul className="divide-y divide-neutral-200 dark:divide-neutral-700 min-h-[80px] border border-neutral-200 dark:border-neutral-700 rounded-md">
                  {releaseIssues && releaseIssues.length === 0 && <li className="p-4 text-neutral-500 text-center">No issues assigned to this release.</li>}
                  {releaseIssues && releaseIssues.map((issue: Issue) => (
                    <li key={issue.id} className="flex items-center gap-4 px-4 py-3 group">
                      <div className="flex-1 min-w-0">
                        <div className="font-medium text-neutral-800 dark:text-neutral-200 truncate">{issue.title}</div>
                        <div className="flex gap-2 mt-1 text-xs items-center">
                          <span className={`${statusColors[issue.status as keyof typeof statusColors] || 'bg-neutral-200'} px-2 py-0.5 rounded font-semibold`}>{issue.status}</span>
        ...
        {activeTab === 'notes' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h4 className="font-semibold text-neutral-800 dark:text-neutral-200">Auto-Generated Release Notes</h4>
              <Button
                size="sm"
            ...
            {loadingNotes ? (
              ...
              <div className="bg-neutral-50 dark:bg-neutral-800 rounded-lg p-4">
                <div className="text-xs text-neutral-500 mb-2">{notesData.issueCount} issue(s) included</div>
                <div className="prose prose-sm dark:prose-invert max-w-none whitespace-pre-wrap font-mono text-sm">
                  {notesData.notes}
                </div>
              </div>
            ) : (
              <div className="text-neutral-500 text-center py-8">No notes generated.</div>
            )}
        ...
        {activeTab === 'git' && (
          <div className="space-y-4">
            <h4 className="font-semibold text-neutral-800 dark:text-neutral-200">Git Integration</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">Commit SHA</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">Repository URL</label>
                <input
                  type="url"
        ...
        {activeTab === 'actions' && (
          <div className="space-y-6">
            <h4 className="font-semibold text-neutral-800 dark:text-neutral-200">Release Actions</h4>

            {/* Deploy Section */}
            <div className="border border-neutral-200 dark:border-neutral-700 rounded-lg p-4">
              <h5 className="font-medium text-neutral-700 dark:text-neutral-300 mb-2 flex items-center gap-2">
                <RocketLaunchIcon className="h-5 w-5 text-green-500" /> Deploy Release
              </h5>
              <p className="text-sm text-neutral-500 mb-3">Trigger a deployment for this release.</p>
              <Button
                onClick={async () => {
                  setDeploying(true);
            ...
            <div className="border border-orange-200 dark:border-orange-700 rounded-lg p-4 bg-orange-50 dark:bg-orange-900/20">
              ...
              <p className="text-sm text-neutral-600 dark:text-neutral-400 mb-3">Create a new release based on this one for rollback purposes.</p>
              <Button
                variant="secondary"
                onClick={async () => {
