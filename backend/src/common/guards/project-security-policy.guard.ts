...
export const SkipPolicyCheck = () => {
    return (target: object, key?: string | symbol, descriptor?: PropertyDescriptor) => {
        if (descriptor) {
            Reflect.defineMetadata(SKIP_POLICY_CHECK, true, descriptor.value);
        }
        return descriptor;
    };
};
...
export class ProjectSecurityPolicyGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        const projectId = request.params?.id || request.params?.projectId || request.query?.projectId;
        ...
        if (violations.length > 0) {
            ...
            const messages: Record<string, string> = {
                '2FA_NOT_ENABLED': 'This project requires Two-Factor Authentication. Please enable 2FA in your security settings.',
                'PASSWORD_EXPIRED': 'Your password has expired. Please update your password.',
                'IP_NOT_ALLOWED': 'Your IP address is not on the allowlist for this project.',
            };

            const primaryViolation = violations[0];
            const message = messages[primaryViolation] || 'You do not meet the security requirements for this project.';
