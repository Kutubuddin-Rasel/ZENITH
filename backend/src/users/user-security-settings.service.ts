...
export class UserSecuritySettingsService {
    ...
    async update(
        userId: string,
        updates: Partial<Omit<UserSecuritySettings, 'id' | 'userId' | 'createdAt' | 'updatedAt'>>,
        ...
        if (updates.sessionTimeoutMinutes !== undefined) {
            updates.sessionTimeoutMinutes = Math.max(5, Math.min(1440, updates.sessionTimeoutMinutes));
        }
        if (updates.maxConcurrentSessions !== undefined) {
            updates.maxConcurrentSessions = Math.max(1, Math.min(20, updates.maxConcurrentSessions));
        }
