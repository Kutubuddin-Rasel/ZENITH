...
import { AutomationRule, AutomationRuleStatus } from '../entities/automation-rule.entity';
...
describe('AutomationRulesService', () => {
    ...
    const mockRule = {
        ...
        actions: [
            { id: 'a1', type: 'update_field', config: { field: 'resolution', value: 'Fixed' }, order: 1 },
        ],
        isActive: true,
    ...
    describe('CRUD', () => {
        ...
        it('should update rule', async () => {
            ...
            const result = await service.updateRule('rule-1', 'user-1', { name: 'Updated' });
            expect(result.name).toBe('Updated');
        });

        it('should throw NotFoundException on update if rule missing', async () => {
            repo.findOne.mockResolvedValue(null);
            await expect(service.updateRule('r1', 'u1', {})).rejects.toThrow(NotFoundException);
        });
    });
