...
export class WorkflowTransitionsService {
    ...
    async isTransitionAllowed(
        ...
        if (!targetStatus) {
            ...
            return { allowed: false, reason: `Status "${targetStatusName}" is not defined for this project` };
        }
        ...
        const totalRules = await this.repo.count({ where: { projectId, isActive: true } });
        if (totalRules === 0) {
            return { allowed: true };
        }
        ...
        if (transition.allowedRoles?.length && !transition.allowedRoles.includes(userRole)) {
            return {
                allowed: false,
    ...
    async create(dto: CreateTransitionDto): Promise<WorkflowTransition> {
        ...
        if (!targetStatus) {
            throw new NotFoundException(`Target status not found: ${dto.toStatusId}`);
        }
        ...
        if (dto.fromStatusId) {
            ...
            if (!fromStatus) {
                throw new NotFoundException(`From status not found: ${dto.fromStatusId}`);
            }
        }
    ...
    async update(
        id: string,
        updates: Partial<Pick<WorkflowTransition, 'name' | 'description' | 'allowedRoles' | 'conditions' | 'isActive' | 'position'>>,
