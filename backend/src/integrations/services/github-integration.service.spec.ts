...
describe('GitHubIntegrationService', () => {
    ...
    describe('syncRepositories', () => {
        ...
        it('should throw error when integration not found', async () => {
            ...
            await expect(
                service.syncRepositories('non-existent-id'),
            ).rejects.toThrow('GitHub integration non-existent-id not found');
        });
    });
    ...
    describe('listUserRepositories', () => {
        it('should list all user repositories with pagination', async () => {
            ...
            mockFetch.mockResolvedValueOnce({
                ok: true,
                json: async () => [
                    { full_name: 'owner/repo1', name: 'repo1', private: false, description: null },
                    { full_name: 'owner/repo2', name: 'repo2', private: true, description: 'A repo' },
                ],
            });
