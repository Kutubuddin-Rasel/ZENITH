...
export class GitHubAppService {
  ...
  generateAppJWT(): string {
    ...
    try {
      ...
      this.logger.error(
        'Failed to parse private key. Make sure GITHUB_APP_PRIVATE_KEY is a valid PEM-formatted RSA private key.',
      );
      this.logger.error(
  ...
  async getInstallationToken(installationId: string): Promise<string> {
    ...
    if (
      cached &&
      cached.expiresAt.getTime() > Date.now() + this.TOKEN_REFRESH_BUFFER_MS
    ) {
      return cached.token;
    }
  ...
  async deleteInstallation(installationId: string): Promise<void> {
    ...
    this.logger.log(
      `GitHub App installation ${installationId} deleted successfully`,
    );
  }
  ...
  async handleInstallationRepositoriesEvent(
    ...
    if (payload.action === 'added') {
      ...
      integration.config.repositories = [
        ...new Set([...currentRepos, ...newRepos]),
      ];
      this.logger.log(
        `Added ${newRepos.length} repos to integration ${integration.id}`,
      );
  ...
  getInstallationUrl(state?: string): string {
    const appSlug =
      this.configService.get<string>('GITHUB_APP_SLUG') || 'zenith-pm';
