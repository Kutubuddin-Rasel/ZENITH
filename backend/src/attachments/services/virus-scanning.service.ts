...
export class VirusScanningService implements OnModuleInit {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.clamavHost = this.configService.get<string>(
      'CLAMAV_HOST',
      'localhost',
    );
  ...
  async onModuleInit(): Promise<void> {
    if (!this.isEnabled) {
      this.logger.warn(
        'ClamAV virus scanning is DISABLED. Set CLAMAV_ENABLED=true to enable.',
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `ClamAV connected: ${this.clamavHost}:${this.clamavPort}`,
      );
    } catch (error) {
  ...
  async scanFile(
    ...
    if (!this.clamScan) {
      this.logger.error(
        'ClamAV not available - rejecting upload (Fail Closed policy)',
      );
